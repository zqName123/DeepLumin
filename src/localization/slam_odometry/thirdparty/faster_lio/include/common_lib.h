...
template <typename S>
inline Eigen::Matrix<S, 3, 1> VecFromArray(const S (&v)[3]) {
    return Eigen::Matrix<S, 3, 1>(v[0], v[1], v[2]);
}
...
template <typename S>
inline Eigen::Matrix<S, 3, 3> MatFromArray(const S (&v)[9]) {
    Eigen::Matrix<S, 3, 3> m;
    m << v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8];
    return m;
}
...
struct MeasureGroup {
    MeasureGroup() { lidar_.reset(new PointCloudType()); }

    double lidar_bag_time_ = 0;
    double lidar_end_time_ = 0;
    PointCloudType::Ptr lidar_ = nullptr;
    std::deque<ImuSamplePtr> imu_;
};
...
/* plane: nx*x + ny*y + nz*z + d = 0; solve A/D*x + B/D*y + C/D*z = -1, then normalize */
template <typename T>
inline bool esti_plane(Eigen::Matrix<T, 4, 1>& pca_result, const PointVector& point, const T& threshold = 0.1f) {
    if (point.size() < static_cast<size_t>(options::MIN_NUM_MATCH_POINTS)) {
        return false;
    }

    Eigen::Matrix<T, 3, 1> normvec;

    if (point.size() == static_cast<size_t>(options::NUM_MATCH_POINTS)) {
